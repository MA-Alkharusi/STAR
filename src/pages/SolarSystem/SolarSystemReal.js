...
function SolarSystem() {
  useEffect(() => {
    ...
    const mercury = createPlanete(3, mercuryTexture, 28);  //mercury 1/3 the size
    const venus = createPlanete(6, venusTexture, 44);    //Venus = 6km radius (slightly smaller)
    const earth = createPlanete(6.2, earthTexture, 62);      //Earth = 6.3km radius
    const mars = createPlanete(4, marsTexture, 78);       //mars about half the size
    const jupiter = createPlanete(12, jupiterTexture, 100);  //jupiter 11x the size
    ...
    const neptune = createPlanete(7, neptuneTexture, 200);     //slightly smaller than uranus
    const pluto = createPlanete(2.8, plutoTexture, 216);       //slightly larger than 1/6 the size
    ...
    return () => {
      document.getElementById('solar-system-container').removeChild(renderer.domElement);
      renderer.dispose();
    };
  }, []);

  return <div id="solar-system-container" className="solar-system-container" />;
}
